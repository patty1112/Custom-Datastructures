...
class DoublyLinkedList {
    ...
    void bubbleSort(bool (*compare)(T, T));     //Bubble sort, takes arguement of acending or decending order
    void listMergeSort(bool (*compare)(T, T));                       //Merge sort of a list object, also uses another merge sort with pointers
    void assignTail();                          //Assigns tail if not at the end of the list    
    bool acending(T x, T y);                    //Comparitive operators for sorting
    bool decending (T x, T y);
    void twoDLinkedList(ifstream&, int);
    T getMin();
    T getMax();
};
...
template <typename T>
void DoublyLinkedList<T>::bubbleSort(bool (*compare)(T, T)) {        
    ...
    for(int i = 0; i < listSize; i++) {         //For each element in the list
        ...
        for(int j =  1; j < listSize - i; j++) {        //For each element, last i elements are sorted already
            if(compare(currPos->value, currPos->pNext->value)) {        //If the current element is greater than the next,
                T currMax = currPos->value;                //Store the value of the
...
template <typename T>
void DoublyLinkedList<T>::listMergeSort(bool (*compare)(T, T)) {         //Merge sort a linked list
    if(this->listSize <= 1) {                   //If the size is 1 or less, do nothing
        ...
        mergeSort(LHS, compare);                             //Merge sort the lists, takes node pointer arguements, returns the head of the sorted list. (defined in DoublyNode.hpp)
        mergeSort(RHS, compare);

        this->pHead = merge(LHS, RHS, compare);              //The head of the list is the head of the merged lists
...
T DoublyLinkedList<T>::getMin() {
    ...
    int minimum = pHead->value;
    while(curr->pNext != nullptr) {
        curr = curr->pNext;
        if(curr->value < minimum) {
            minimum = curr->value;
        }
    }
    return minimum;
