...
class BinarySearchTree {
    ...
    void printInOrder(TreeNode<T>*);        //Prints the tree in order
    void printPreOrder(TreeNode<T>*);       //Prints the root, then left subtree, then right subtree
    void printPostOrder(TreeNode<T>*);      //Prints the right subtree, the left subtree, then the root
...
BinarySearchTree<T>::~BinarySearchTree() {      //Deallocates the whole binary tree
    ...
    cout << "Deleting the node " << root->value << endl;        //Print the value of the node deleted, verification purposes
...
void BinarySearchTree<T>::printInOrder(TreeNode<T>* root) {
    if(root == nullptr) {       //If the list is empty
        return;             //Return
    }
    printInOrder(root->left);       //If not empty, print the left subtree
...
template <typename T>
void BinarySearchTree<T>::printPreOrder(TreeNode<T>* root) {    //Print the root, then the left subtree, then the right subtree
    ...
    root->visited = true;               //Make the node visited
    printPreOrder(root->left);          //Print the left subtree in preorder
    printPreOrder(root->right);         //Print the right subtree in preorder
    
}


template <typename T>
void BinarySearchTree<T>::printPostOrder(TreeNode<T>* root) {   //Prints the tree, left subtree, then right subtree, then root
    if(root == nullptr) {           //If the tree is empty
        return;                 //Return
    } 
    
    printPostOrder(root->left);         //Print the left subtree in postorder
    printPostOrder(root->right);        //Print the right subtree in post order
...
void deleteSubTree(TreeNode<T>* root) {         //Deletes a subtree given its root
    ...
    cout << "Deleting the node " << root->value << endl;        //Deletes the node
    delete root;            
...
void BinarySearchTree<T>::printDepthOrder() {       //Prints the tree depth first
    this->printPreOrder(this->root);        //Pre order is a type of depth transversal, print preorder
}
