...
class DoublyLinkedList {
    ...
    DoublyNode<T>* pHead;       //Pointer to the start of the list
    DoublyNode<T>* pTail;       //Pointer to the end of the list
    unsigned int listSize;      //tracker for the size of the list
    ...
    T at(const int&);                       //Gets the value of the node at a position
    void assignTail();                      //Iterates through to last node, assigns the tail
...
void DoublyLinkedList<T>::remove(const int POS) {
    ...
    if(POS == 0) {                      //If postion is 0
        pHead = currPos->pNext;             //Delete the head and move it to the next node
        currPos->pNext->pPrev = nullptr;
        delete currPos;
        currPos = nullptr;
        listSize -= 1;              //Update listSize
        return;
...
template <typename T>
T DoublyLinkedList<T>::at(const int& position) {
    DoublyNode<T>* curr = pHead;                      //Pointer to track position
