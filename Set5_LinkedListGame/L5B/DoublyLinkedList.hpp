...
void DoublyLinkedList<T>::bubbleSort() {
    DoublyNode<T>* currPos;                     //Make a node pointer
    for(int i = 0; i < listSize; i++) {         //For each node in the list
        currPos = pHead;                        //Set curr as the head  
        bool swap = false;                      //There have not been swaps yet
        for(int j =  1; j < listSize - i; j++) {        //For each node, last i nodes are sorted
            if(currPos->value > currPos->pNext->value) {    //If the node is greater than the next node
                int oldMax = currPos->value;                //Store the value
                currPos->value = currPos->pNext->value;     //Swap the value of the left node
                currPos->pNext->value = oldMax;             //Make the next node hold the value of the maximum
                swap = true;                                //There has been a swap
            }
            currPos = currPos->pNext;               //move to the next node
        }
        if(swap == false) {             //If no nodes got swaped,
            this->assignTail();         //Make the tail the last node
            return;                     //return
        }
    }
}
