...
struct DoublyNode {
    ...
    DoublyNode<T>* pPrev;
    void mergeSort(DoublyNode<T>*&);
    void splitLinkedList(DoublyNode<T>*&, DoublyNode<T>*&, DoublyNode<T>*&);
    DoublyNode<T>* merge(DoublyNode<T>*&, DoublyNode<T>*&);
};

template <typename T>
DoublyNode<T>* merge(DoublyNode<T>*& LHS, DoublyNode<T>*& RHS) {    //Merges two lists and returns a pointer to the head
    ...
    if(LHS->value <= RHS->value) {                  //If the value of the left is smaller than the right,
        mergedHead = LHS;                           //Make the head the left pointer
        mergedHead->pNext = merge(LHS->pNext, RHS);     //Then merge the next node and RHS
    } else {
        mergedHead  = RHS;                              //Else make the Head the right hand side
        mergedHead->pNext = merge(LHS, RHS->pNext);     //Merge the left hand side and the next node of the right
    }
...
template <typename T>
void mergeSort(DoublyNode<T>*& listHead) {                  //MergeSort, but takes just the pointer as arguement. 
    if(listHead == nullptr || listHead->pNext == nullptr) {     //If the node is the last one, do nothing.
        ...
        mergeSort(LHS);                             //Sort the two sides
        mergeSort(RHS);
        listHead = merge(LHS, RHS);             //Make the head of the list the merged list head
